...
# --- Compatibility patch for newer transformers cache API ---
try:
...
if DynamicCache is not None and not hasattr(DynamicCache, "seen_tokens"):
    ...
    def seen_tokens(self):
        """
        Backwards-compatible alias used by older remote code.
        Approximate the old `seen_tokens` by using the current cache length.
        """
        if hasattr(self, "get_seq_length"):
            return self.get_seq_length()
        try:
            return len(self.get_usable_cache_values()[0][0])
        except Exception:
            return None
