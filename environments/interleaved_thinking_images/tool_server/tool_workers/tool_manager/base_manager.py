...
class ToolManager(object):
    ...
    def init_online_tools(self, controller_url_location=None):
        ...
        with self.disable_proxy():
            if self.controller_addr is not None and isinstance(self.controller_addr, str):
                ...
                try:
                    ...
                    logger.info(f"refresh_all_workers status={ret.status_code}, text={str(ret.text)[:200]}")
                except Exception as e:
                    logger.error(f"refresh_all_workers failed: {e}")

                # retry list_models (SAM2 may register late)
                models = []
                for t in range(30):  # retry up to 30 seconds
                    try:
                        ret = requests.post(self.controller_addr + "/list_models")
                        logger.info(f"list_models status={ret.status_code}, text={str(ret.text)[:200]}")
                    ...
                    # wait until BioMedParseTextSeg is registered (it loads slowly), or timeout
                    required = {"SegmentRegionAroundPoint", "BioMedParseTextSeg"}

                    # Option A: stop only when ALL required tools exist
                    if required.issubset(set(models)):
                        break
        ...
        miss_tool = []
        for tool in ["ZoomInSubfigure","DrawHorizontalLineByY","OCR","DrawVerticalLineByX","SegmentRegionAroundPoint", "BioMedParseTextSeg","Point"]:
    ...
    def call_tool(self,tool_name,params):
        # If tool may have registered after ToolManager init, refresh once
        if tool_name not in self.available_offline_tools and tool_name not in self.available_online_tools:
            try:
                self.init_online_tools(self.controller_url_location)
                self.init_online_tool_addr_dict()
                self.available_tools = self.available_offline_tools + self.available_online_tools
